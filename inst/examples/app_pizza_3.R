...
# SurveyJS schema with pricing logic
pizza_schema <- list(
  ...
  calculatedValues = list(
    # Price table for each size
    list(name = "size_price", expression = "({'Small':8,'Medium':11,'Large':14,'XL':17}[size])"),
    list(name = "topping_price", expression = "0.9"), # Each topping costs $0.90
    list(name = "gf_price", expression = "gluten_free ? 2 : 0") # Gluten-free surcharge
  ),
  pages = list(
    # Customer info
    list(
        ...
        list(
          ...
          templateElements = list(
            ...
            list(
              type = "checkbox",
              name = "toppings",
              title = "Extra Toppings (up to 4, $0.90 each)",
            ...
            list(
              ...
              placeHolder = "No onions, cut in squares, etc.",
              inputType = "textarea"
            ),
            # Per-pizza price
            list(
              type = "expression",
              name = "pizza_price",
              title = "<b>Price for this pizza:</b>",
              displayStyle = "currency",
              currency = "USD",
              expression =
                "({'Small':8,'Medium':11,'Large':14,'XL':17}[size])
    ...
    list(
      ...
      elements = list(
        list(
          ...
          expression =
            "join(map({pizzas}, function(p, i) {
               ...
               '<br><b>Pizza Price: $' + (({'Small':8,'Medium':11,'Large':14,'XL':17}[p.size])
        ...
        list(
          type = "expression",
          name = "cart_total",
          title = "<span style='font-size:18px;'><b>Total Pizza(s):</b></span>",
          displayStyle = "currency",
          currency = "USD",
          expression =
            "sum(map({pizzas}, function(p) {
               ( {'Small':8,'Medium':11,'Large':14,'XL':17}[p.size] || 0 )
               + (p.toppings ? count(p.toppings) * 0.9 : 0)
               + (p.gluten_free ? 2 : 0)
             }))"
        ),
        ...
        list(
          ...
          displayStyle = "currency",
          currency = "USD",
          visibleIf = "{delivery_type} = 'Delivery'",
          expression = "4"
        ),
        # Grand total
        list(
          type = "expression",
          name = "grand_total",
          title = "<span style='font-size:20px;color:#fc5c2f'><b>Order Total:</b></span>",
          displayStyle = "currency",
          currency = "USD",
          expression =
            "sum(map({pizzas}, function(p) {
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      .survey-container { max-width: 570px; margin: 2em auto; background: #fff; border-radius: 22px; box-shadow: 0 6px 32px #aaa3; padding: 36px 30px 16px 30px;}
      h2, h3 { text-align: center; }
      .sv_progress-buttons__page-title { font-weight: bold; }
      .sv_p_root { background: #fff8f0 !important; }
      .sv_q_text_root { font-size: 1.1em !important; }
      .sv-question__title, .sv_q_title { font-weight: 500 !important; color: #333 !important; }
      .sv-paneldynamic__footer { display:none; } /* Hide the 'required' message under pizza cart */
    "))
  ),
