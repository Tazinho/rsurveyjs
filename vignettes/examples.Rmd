...
`surveyjs` offers a rich set of validators that you can naturally provide as part of the schema.
    ...
    list(
      ...
      elements = list(
        ...
        list(
          type = "text",
          name = "code",
          title = "Enter your employee code (AAA-999)",
          validators = list(
            list(type = "regex", regex = "^[A-Z]{3}-[0-9]{3}$", text = "Must match AAA-999 format")
          )
        ),
        list(
          type = "text",
          ...
          inputType = "number",
          validators = list(
            list(type = "numeric", minValue = 18, maxValue = 99)
          )
        )
      )
    )
...
And of course surveyjs provides also conditional logic.
...
surveyjs(schema = list(
  ...
  questions = list(
    ...
    list(
      ...
      name = "whyNot",
      title = "Why not?",
      visibleIf = "{satisfied} = 'No'"
    )
  )
))
```

## Pre- and Post-Render Hooks (Advanced)

**Pre-render hooks** are JavaScript snippets that run **before** the form is displayed. Use them to attach behavior like validation, event listeners, or model logic.
...
**Post-render hooks** run **after** the form has been rendered on screen. Use them to style the container, inspect the DOM, or log information.
...
surveyjs(
  ...
  post_render_hook = "
    ...
    console.log('Survey rendered with schema:', x.schema);
  "
)
```

## SurveyJS Event Listeners (Advanced)

SurveyJS supports many event listeners to let you react to user input, validation, and survey lifecycle changes.

Event listeners are attached inside the `pre_render_hook`. You can use them to:

* React to value changes (`onValueChanged`)

* Add custom validation (`onValidateQuestion`)

* Log when the survey is completed (`onComplete`), and more.
...
surveyjs(
  schema = list(
    title = "Track input",
    questions = list(
      list(type = "text", name = "email", title = "Enter your email")
    )
  ),
  pre_render_hook = "
    survey.onValueChanged.add(function(sender, options) {
      if (options.name === 'email') {
        console.log('User typed email:', options.value);
      }
    });
  "
)
...
For a full list of available events, see the
[SurveyJS Event Documentation](https://surveyjs.io/form-library/documentation/api-reference/survey-data-model#events).
